  ...
  return (
    <div className="snap">
      <main className="shell shell-wide">
        ...
        {/* ── MVP³ reveal — held back until there is a result to attach it to ── */}
        <section className="sec-rule">
          <div className="reveal">
            <span className="lbl">{t(UI.revealKicker, lang)}</span>
            <div className="rt">{t(UI.revealTitle, lang)}</div>
            <p className="rb">{t(UI.revealBody, lang)}</p>
            <ol>
              {UI.revealLenses.map(([name, desc], n) => (
                <li key={n}>
                  <span className="rn">0{n + 1}</span>
                  <span className="rl">{t(name, lang)}</span>
                  <span className="rd">{t(desc, lang)}</span>
                </li>
              ))}
            </ol>
            <p className="rc">{t(UI.revealClose, lang)}</p>
          </div>
        </section>
